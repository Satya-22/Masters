...
object fp3:
  ...
  def member(a: Int, xs: List[Int]): Boolean = xs match {
    ...
    case Nil => false
    case y::ys => if a == y then true else member(a,ys)
  ...
  def delete1[X](x: X, ys: List[X]): List[X] = ys match {
     ...
     case Nil => Nil
     case y::Nil => if x == y then Nil else y::Nil
     case y::yy  => if x == y then delete1(x,yy) else y :: delete1(x,yy)
    // TODO: Provide definition here.
  }
    //throw UnsupportedOperationException()
  ...
  def removeDupes1[X](xs: List[X]): List[X] = xs match {
    ...
    case Nil => Nil
    case y::Nil => y::Nil
    case y::yy => if y == yy.head then removeDupes1(yy) else y::removeDupes1(yy)
  ...
  def removeDupes2[X](xs: List[X]): List[(Int, X)] = {
    ...
    def temp [X] (tmp:X, ys:List[X],count:Int): List[(Int,X)] = ys match {
    
      case Nil => if (tmp != Nil) then (count,tmp)::Nil else Nil
      case k::ks => if (k != tmp) then (count,tmp)::temp(k,ks,1) else temp(k,ks,count+1)
    }

    xs match {
      case Nil => Nil
      case l::Nil => List((1,l))
      case t::ts => temp(t,ts,1)
    }
